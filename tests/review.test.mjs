...
test('全部诗词都有独立的中国风场景图片',()=>{const files=new Set(fs.readdirSync(new URL('../public/images/',import.meta.url)).filter(x=>x.endsWith('.png')));assert.equal(files.size,poems.length);for(const poem of poems)assert.ok(files.has(`${poem.id}.png`),`${poem.title} 缺少场景图片`)});
